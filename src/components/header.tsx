...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 flex justify-center pt-4 sm:pt-6 pb-4 px-4 pointer-events-none transition-transform">
      <div className="pointer-events-auto flex items-center justify-between bg-card/75 backdrop-blur-xl border border-border/50 rounded-full px-4 sm:px-6 h-14 w-full max-w-[1400px] shadow-xl shadow-black/5 dark:shadow-black/20 ring-1 ring-white/10 dark:ring-white/5">
        {/* Brand Logo */}
        <Link
          to="/"
        ...
        <div className="flex items-center gap-1 sm:gap-1.5">
          <div className="hidden sm:flex items-center gap-1 mr-1 sm:mr-2 sm:border-r border-border/50 sm:pr-2">
            <a
              ...
              rel="noreferrer"
              aria-label="GitHub"
              className="p-2 rounded-full text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-all"
            >
          ...
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent
              side="right"
              className="w-[280px] sm:w-[320px] bg-card/95 backdrop-blur-2xl border-l-border/50"
            >
